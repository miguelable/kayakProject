...
const uint16_t PixelCount =
    280; // make sure to set this to the number of pixels in your strip
const uint16_t PixelPin =
    4; // make sure to set this to the correct pin, ignored for Esp8266
const uint16_t AnimCount = 1; // we only need one
const uint16_t TailLength =
    30; // length of the tail, must be shorter than PixelCount
const float MaxLightness =
    0.4f; // max lightness at the head of the tail (0.5f is full bright)
...
void DrawTailPixels() {
  ...
  for (uint16_t index = 0; index < strip.PixelCount() && index <= TailLength;
       index++) {
